...
Although the exploratory plots and main-effects model indicate that only factors A and D meaningfully influence flight time, these conclusions rely on a model that does not include interaction terms. Because factors in a full factorial experiment may contribute through interactions even when their main effects appear small, we also compared the full interaction model to reduced models that removed each factor and all of its associated interaction terms (see Appendix for code). These nested ANOVA comparisons quantify the total contribution of each factor to model fit. Removing factor A or factor D led to substantial increases in residual error ($\Delta$adj-$R^2$ = 0.259 and 0.312, respectively), reaffirming that both factors are highly influential. Removing factors B or C also significantly worsened the model ($\Delta$adj-$R^2$ = 0.193 and 0.150), suggesting that they participate in interaction patterns even though their main effects alone were not statistically significant. All p-values for each of the nested F-tests between the full model and the models with factors involvind a, b, c, or d removed were very small (all of them smaller than .01).
Thus, while the simple main-effects model highlights A and D as the most important design features, the broader model-comparison results show that all four factors contribute to explaining flight time to varying degrees, with D and A having the strongest overall influence. Full details and output tables for these comparisons are provided in the Appendix.
